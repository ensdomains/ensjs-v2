...
import ENS, { namehash } from '../index.js'
...
describe('Blockchain tests', () => {
  ...
  describe('Test contract and Web3 setup', () => {
    ...
    //   const ethOwner = await ens.getName('eth').getOwner()
    //   expect(ethOwner).toBe(baseRegistrar)

    //   //const reverse = namehash('reverse')
    //   const reverseOwner = await ens.getName('eth').getOwner('reverse')
    //   expect(reverseOwner).toBe(accounts[0])

    //   //const reverseNode = namehash('addr.reverse')
    //   const reverseNodeOwner = await ens.getName('addr.reverse').getOwner()
